	...
	public static void main(String[] args) throws IOException {
		...
		if(local.exists()) {
			if(!local.isDirectory()) {
				System.err.println("error " + localPath);
				System.exit(0);
			}
		} else {
			local.mkdirs();
		...
		String regex = "^.*[0-9]{4}\\-[0-2]{2}\\-[0-9]{2}\\.(csv|xls|xlsx)$";
		//FTPFile[] listAllFile = client.listAllFile(ftpPath,regex);
		FTPFile[] listAllFile = client.listAllFile(ftpPath);
		for (FTPFile ftpFile : listAllFile) {
			System.err.println(ftpFile);
		}
...
//			pool.execute(new HandleGcThread(entry.getKey(), entry.getValue(), bean, fileEncoding));
			pool.execute(new HandleGcThreadWithTrans(entry.getKey(), entry.getValue(), context, fileEncoding));
...
//			int insert = bean.insert("com.cmdi.dao.EpDataDao.insertglobalcellenbnumtablewithscenarioandvendor", map);
//			int insert2 = bean.insert("com.cmdi.dao.EpDataDao.updateglobalcellenbnumtablewithscenarioandvendor", map);
//			System.out.println("insert 全国 基站、小区 rows " + (insert + insert2));
			int delete = bean.delete("com.cmdi.dao.EpDataDao.deleteglobalcellenbnum", map);
			System.out.println("delete 全国 基站、小区 rows " + delete);
			int insert = bean.insert("com.cmdi.dao.EpDataDao.insertglobalcellenbnumtablewithscenarioandvendor2", map);
